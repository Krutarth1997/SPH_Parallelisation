...
void ForcesFluid(int n_particles, const std::vector<std::vector<int>>& neighbor_ids, const std::vector<std::vector<float>>& distances, const Eigen::MatrixXf& positions, 
                ...
                float artificial_surface_tension_curvature, float SURFACE_TENSION, float density_gas, float cw,const Eigen::Matrix<float, 1, 3>& CONSTANT_FORCE,const Eigen::Matrix<float, 1, 3>& velocity_gas, Eigen::MatrixXf gradColorField, int count);
